...
class InverseBackdoorSets(Predictor):
    ...
    def __init_phase(self, count, solver):
        env.comm.debug(1, 0, 'generating init cases...')
        ...
        env.comm.debug(1, 0, 'has been solved %d init cases' % len(solutions))
        if count != len(solutions):
            env.comm.debug(0, 0, 'warning! count != len(init_solved)')
    ...
    def __main_phase(self, backdoor, inited, solver):
        ...
        if len(inited) != len(solved):
            env.comm.debug(0, 0, 'warning! len(init_solved) != len(solved)')

        return solved, time

    def compute(self, backdoor, cases, count, **kwargs):
        init, main = env.solvers['init'], env.solvers['main']
        env.comm.d_debug(1, 0, 'compute for backdoor: %s' % backdoor)
        env.comm.d_debug(1, 0, 'use time limit: %s' % main.get('tl'))
    ...
    def calculate(self, backdoor, compute_out):
        ...
        if self.corrector is not None:
            env.comm.debug(1, 0, 'correcting time limit...')
            tl, dis_count = self.corrector.correct(cases, tl)
            log += 'corrected time limit: %f\n' % tl
            env.comm.d_debug(1, 0, 'new time limit: %f' % tl)

            env.comm.debug(1, 0, 'correcting time stat...')
            time_stat['DISCARDED'] = dis_count
            time_stat['DETERMINATE'] -= dis_count
            env.comm.d_debug(1, 0, 'new time stat: %s' % time_stat)
        ...
        env.comm.debug(1, 0, 'value: %.7g' % value)
